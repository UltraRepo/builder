...
export class QdrantVectorStore implements IVectorStore {
	...
	constructor(
		...
		const parsedUrl = QdrantVectorStore.parseQdrantUrl(url)
		...
		if (customCollectionName && customCollectionName.trim().length > 0) {
	 ...
	 */
	private static parseQdrantUrl(url: string | undefined): string {
		...
		if (!trimmedUrl.startsWith("http://") && !trimmedUrl.startsWith("https://") && !trimmedUrl.includes("://")) {
			...
			return QdrantVectorStore.parseHostname(trimmedUrl)
		}
	 ...
	 */
	private static parseHostname(hostname: string): string {
	...
	/**
	 * Lists all collections from the Qdrant server
	 * @param url Qdrant server URL
	 * @param apiKey Optional API key
	 * @returns Promise resolving to an array of collection names
	 */
	static async listCollections(url: string, apiKey?: string): Promise<string[]> {
		try {
			const parsedUrl = QdrantVectorStore.parseQdrantUrl(url)
			const urlObj = new URL(parsedUrl)
			...
			console.error("[QdrantVectorStore] Failed to list collections:", error)
			return []
		}
	}
}
