...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "requestHuggingFaceModels":
			...
			break
			break
		case "getQdrantCollections":
			try {
				const collections = await QdrantVectorStore.listCollections(
					message.values?.url,
					message.values?.apiKey,
				)
				provider.postMessageToWebview({
					type: "qdrantCollections",
					collections,
				})
			} catch (error) {
				provider.postMessageToWebview({
		...
		case "openExternal":
			...
			break
		case "openHome":
			// Open the UltraRepo Home webview panel
			await vscode.commands.executeCommand("ultrarepo.__openHomeInline")
			break
		case "checkpointDiff":
		...
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "ultrarepo.")
			break
		...
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("ultrarepo.ghost.reload")
			break
		// kilocode_change end
		...
		case "saveCodeIndexSettingsAtomic": {
			...
			try {
				...
				const globalStateConfig = {
					...
					codebaseIndexNeoCoderUrl: settings.codebaseIndexNeoCoderUrl,
				...
				if (settings.codeIndexNeoCoderApiKey !== undefined) {
					await provider.contextProxy.storeSecret("codeIndexNeoCoderApiKey", settings.codeIndexNeoCoderApiKey)
				}
		...
		case "requestCodeIndexSecretStatus": {
			...
			const hasNeoCoderApiKey = !!(await provider.context.secrets.get("codeIndexNeoCoderApiKey"))
