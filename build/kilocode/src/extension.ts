	...
	constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri) {
		...
		this.panel.webview.onDidReceiveMessage(async (msg: any) => {
			...
			if (msg.type === 'launchGraphViewer') {
				...
				vscode.commands.executeCommand('builder.showDependencyGraph')
				return
			}
...
<body>
	...
	<div class="wrap">
		<section id="home" class="active"><div class="card"><h2>Home</h2><p class="muted">Welcome to AirVeo App Builder - your AI coding assistant. Choose a feature from the navigation above to get started.</p><button onclick="vscode.postMessage({type:'run-cmd', cmd:'builder.showDependencyGraph'})" style="margin-top:16px;padding:10px 20px;background:var(--primary);color:#fff;border:none;border-radius:8px;cursor:pointer;">Open Dependency Graph</button></div></section>
			<section id="graph"><div class="card"><h2>Dependency Graph</h2><p class="muted">Visualize and explore your project dependencies. Click the button below to open the graph.</p><button onclick="vscode.postMessage({type:'run-cmd', cmd:'builder.showDependencyGraph'})" style="margin-top:16px;padding:10px 20px;background:var(--primary);color:#fff;border:none;border-radius:8px;cursor:pointer;">Open Dependency Graph</button>
		...
		<section id="commands"><div class="card"><h2>Commands</h2><p class="muted">Access AirVeo App Builder commands and features. Use the command palette (Ctrl+Shift+P) to explore available commands.</p></div></section>
			<section id="mcp"><div class="card"><h2>MCP Services</h2><p class="muted">Manage and configure Model Context Protocol services for enhanced AI capabilities.</p><button onclick="vscode.postMessage({type:'run-cmd', cmd:'builder.mcpButtonClicked'})">Open MCP Settings</button></div></section>
			<section id="settings"><div class="card"><h2>Settings</h2><p class="muted">Configure AirVeo App Builder settings and preferences.</p><button onclick="vscode.postMessage({type:'run-cmd', cmd:'builder.settingsButtonClicked'})">Open Settings</button></div></section>
	</div>
	<script nonce="${nonce}">
...
export async function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('builder.__openHomeInline', () => {
	...
	if (!context.globalState.get("firstInstallCompleted")) {
		...
		try {
			await vscode.commands.executeCommand("builder.SidebarProvider.focus")
			...
			await vscode.commands.executeCommand(
				...
				"AirVeo.builder#ultrarepoWalkthrough",
				false,
			)
		} catch (error) {
