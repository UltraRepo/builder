/*
 * registerCommands.ts
 * Centralized command registration for UltraRepo / KiloCode.
 * This file must not contain copied human-readable prompts; keep only valid TypeScript.
 */
...
export const registerCommands = (options: RegisterCommandOptions) => {
	...
	for (const [id, callback] of Object.entries(getCommandsMap(options))) {
		...
		outputChannel.appendLine(`[Commands] Registering command: ${command}`)
...
const getCommandsMap = ({ context, outputChannel }: RegisterCommandOptions): Record<CommandId, any> => ({
	...
	home: async () => {
		try {
			await vscode.commands.executeCommand('builder.__openHomeInline')
			outputChannel.appendLine('Home webview opened successfully')
		} catch (error) {
			outputChannel.appendLine(`Error opening home webview: ${error}`)
		}
	},
})
