...
- Safety rules (must follow)
  ...
  - Git modules rule: Do not change .gitmodules location — leave it at repo root. If submodule paths moved locally, prefer creating runtime symlink or updating local config; do not move .gitmodules.

  - Commit policy (important):
    - Do NOT run git add/commit/push automatically. When making edits on request, modify files in the workspace but leave changes unstaged.
    - Only commit and push when the user explicitly asks and provides a commit message. This prevents accidental commits of generated or experimental changes.
...
- Project-specific conventions
  ...
  - Editing guidance: Always change source in rebrand/ then run the build script; build/ is generated and will be overwritten. Avoid editing .ultrarepo/upstream/* and upstream/* files.
...
- When in doubt
  ...
  - For additional guidance: Open an issue or PR describing changes; include a reproducible smoke build log when relevant.
