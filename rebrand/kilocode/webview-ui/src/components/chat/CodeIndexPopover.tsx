...
interface LocalCodeIndexSettings {
	...
	codebaseIndexQdrantCollection: string
...
export const CodeIndexPopover: React.FC<CodeIndexPopoverProps> = ({
	...
	useEffect(() => {
		if (codebaseIndexConfig) {
			const settings = {
				...
				codebaseIndexQdrantCollection: codebaseIndexConfig.codebaseIndexQdrantCollection || "",
	...
	// State for available Qdrant collections
	const [availableCollections, setAvailableCollections] = useState<string[]>([])
	const [isLoadingCollections, setIsLoadingCollections] = useState(false)

	const handleGetQdrantCollections = () => {
		setIsLoadingCollections(true)
		vscode.postMessage({
			type: "getQdrantCollections",
			values: {
				url: currentSettings.codebaseIndexQdrantUrl || DEFAULT_QDRANT_URL,
				apiKey: currentSettings.codeIndexQdrantApiKey,
			},
		})
	}

	// Listen for Qdrant collections response
	useEffect(() => {
		const handleMessage = (event: MessageEvent) => {
			if (event.data.type === "qdrantCollections") {
				setIsLoadingCollections(false)
				if (event.data.error) {
					// Show error (could add a toast or error state here)
					console.error("Failed to fetch collections:", event.data.error)
				} else {
					setAvailableCollections(event.data.collections || [])
				}
			}
		}
						...
						<div className="mt-4">
							...
							{isSetupSettingsOpen && (
								<div className="mt-4 space-y-4">
									...
									<div className="space-y-2">
										<label className="text-sm font-medium">
											{t("settings:codeIndex.qdrantCollectionLabel")}
										</label>
										<div className="flex gap-2">
											<div className="flex-1">
												<VSCodeTextField
													value={currentSettings.codebaseIndexQdrantCollection || ""}
													onInput={(e: any) =>
														updateSetting("codebaseIndexQdrantCollection", e.target.value)
													}
													placeholder={t("settings:codeIndex.qdrantCollectionPlaceholder")}
													className="w-full"
												/>
											</div>
											<VSCodeButton
												...
												onClick={handleGetQdrantCollections}>
												{isLoadingCollections ? (
													<span className="codicon codicon-loading codicon-modifier-spin" />
												) : (
													t("settings:codeIndex.getCollectionsButton")
												)}
											</VSCodeButton>
										</div>
										{availableCollections.length > 0 && (
											<div className="mt-2">
												<VSCodeDropdown
													...
													<VSCodeOption value="">
														{t("settings:codeIndex.selectCollectionPlaceholder")}
													</VSCodeOption>
													{availableCollections.map((collection) => (
														<VSCodeOption key={collection} value={collection}>
															{collection}
														</VSCodeOption>
													))}
												</VSCodeDropdown>
