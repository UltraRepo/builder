...
export class CodeIndexConfigManager {
	...
	private neoCoderUrl?: string
	private neoCoderApiKey?: string
	...
	private _loadAndSetConfiguration(): void {
		...
		const neoCoderApiKey = this.contextProxy?.getSecret("codeIndexNeoCoderApiKey") ?? ""
		...
		this.qdrantCollection = codebaseIndexQdrantCollection ?? ""
		this.neoCoderUrl = codebaseIndexNeoCoderUrl ?? ""
		this.neoCoderApiKey = neoCoderApiKey ?? ""
	...
	public async loadConfiguration(): Promise<{
		...
		const previousConfigSnapshot: PreviousConfigSnapshot = {
			...
			qdrantCollection: this.qdrantCollection ?? "",
			neoCoderUrl: this.neoCoderUrl ?? "",
			neoCoderApiKey: this.neoCoderApiKey ?? "",
		}
	...
	doesConfigChangeRequireRestart(prev: PreviousConfigSnapshot): boolean {
		...
		const prevQdrantCollection = prev?.qdrantCollection ?? ""
		const prevNeoCoderUrl = prev?.neoCoderUrl ?? ""
		const prevNeoCoderApiKey = prev?.neoCoderApiKey ?? ""
		...
		const currentQdrantCollection = this.qdrantCollection ?? ""
		const currentNeoCoderUrl = this.neoCoderUrl ?? ""
		const currentNeoCoderApiKey = this.neoCoderApiKey ?? ""
		...
		if (
			...
			prevQdrantCollection !== currentQdrantCollection
		) {
			return true
		}

		if (prevNeoCoderUrl !== currentNeoCoderUrl || prevNeoCoderApiKey !== currentNeoCoderApiKey) {
			return true
		}
	 ...
	 */
	public get qdrantConfig(): { url?: string; apiKey?: string; collection?: string } {
		return {
			url: this.qdrantUrl,
	...
	/**
	 * Gets the current NeoCoder configuration
	 */
	public get neoCoderConfig(): { url?: string; apiKey?: string } {
		return {
			url: this.neoCoderUrl,
			apiKey: this.neoCoderApiKey,
		}
	}
