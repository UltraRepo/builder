...
describe("QdrantVectorStore", () => {
	...
	it("should use custom collection name when provided", () => {
		...
		const vectorStoreWithCustomName = new QdrantVectorStore(
			mockWorkspacePath,
			mockQdrantUrl,
			mockVectorSize,
			mockApiKey,
		...
		expect((vectorStoreWithCustomName as any).collectionName).toBe(customName)
	})

	it("should fallback to hashed path if custom collection name is empty/whitespace", () => {
		const vectorStoreWithEmptyName = new QdrantVectorStore(
			mockWorkspacePath,
			mockQdrantUrl,
			mockVectorSize,
			mockApiKey,
		...
		// Access private member for testing
		expect((vectorStoreWithEmptyName as any).collectionName).toBe(expectedCollectionName)
	})

	describe("listCollections", () => {
		it("should list collections successfully", async () => {
			const mockCollections = [{ name: "collection1" }, { name: "collection2" }]
				; (QdrantClient as unknown as ReturnType<typeof vi.fn>).mockImplementation(() => ({
					getCollections: vi.fn().mockResolvedValue({ collections: mockCollections }),
				}))

			const collections = await QdrantVectorStore.listCollections(mockQdrantUrl)
